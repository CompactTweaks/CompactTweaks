...
$script:Tweaks = @(
    ...
    @{ Id = 'powerplan'; Category = 'Performance'; Name = 'High-performance power plan'; Risk = 'Medium'; Recommended = $false
       Desc = 'Creates and activates an Ultimate/High Performance power plan (never throttles the CPU). Best on desktops; on a laptop it costs battery life and adds heat. Undo switches back to your previous plan and deletes the new one.'
       Apply = {
...
function Invoke-TweakApply {
    ...
    $already = Test-TweakApplied $T
    if ($already -and $script:State.ContainsKey($T.Id)) {
        Write-Log ('Already applied: ' + $T.Name)
        return
    }
...
function Build-TweakList {
    ...
    foreach ($t in $script:Tweaks) {
        ...
        $risk.Text = $t.Risk
        $risk.Width = 70
        $risk.TextAlignment = 'Right'
        $risk.VerticalAlignment = 'Center'
        if ($t.Risk -eq 'Low') { $risk.Foreground = New-Brush '#6B7280' } else { $risk.Foreground = New-Brush '#F59E0B' }
        [System.Windows.Controls.DockPanel]::SetDock($risk, [System.Windows.Controls.Dock]::Right)
...
function Update-Statuses {
    foreach ($t in $script:Tweaks) {
        ...
        $a = Test-TweakApplied $t
        if ($null -eq $a) {
            $row.Status.Text = 'One-time'
            $row.Status.Foreground = New-Brush '#A78BFA'
        } elseif ($a) {
            $row.Status.Text = 'Applied'
            $row.Status.Foreground = New-Brush '#4ADE80'
        } else {
            $row.Status.Text = 'Not applied'
            $row.Status.Foreground = New-Brush '#6B7280'
        }
    }
    Update-UI
}
